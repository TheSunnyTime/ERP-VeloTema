                        ...
                        with transaction.atomic():
                            for i, row in enumerate(reader, start=2):
                                try:
                                    ...
                                    try:
                                        if not csv_wholesale_price_str: 
...
def employee_salary_report_view(request):
    ...
    if not selected_year or not selected_month or not (1 <= selected_month <= 12):
        ...
        default_report_year = last_day_of_previous_month.year
        default_report_month = last_day_of_previous_month.month
        ...
        if is_redirect_needed:
            return HttpResponseRedirect(reverse('utils:my_salary_report') + f'?year={selected_year}&month={selected_month}')

    start_of_selected_period = date(selected_year, selected_month, 1)
    
    accrued_before_period_data = SalaryCalculation.objects.filter(
        employee=report_user, period_date__lt=start_of_selected_period
    ).aggregate(total=Sum('total_calculated_amount'))
    total_accrued_before_period = accrued_before_period_data['total'] or Decimal('0.00')
    
    paid_in_previous_years_data = SalaryPayment.objects.filter(
        employee=report_user, payment_for_year__lt=selected_year
    ).aggregate(total=Sum('amount_paid'))
    total_paid_in_previous_years = paid_in_previous_years_data['total'] or Decimal('0.00')
    
    paid_in_current_year_before_month_data = SalaryPayment.objects.filter(
        employee=report_user, payment_for_year=selected_year, payment_for_month__lt=selected_month
    ).aggregate(total=Sum('amount_paid'))
    ...
    salary_payments = SalaryPayment.objects.filter(
        employee=report_user, 
        payment_for_year=selected_year, 
        payment_for_month=selected_month
    ...
    if next_month_date.year > today.year or (next_month_date.year == today.year and next_month_date.month > today.month):
    ...
    context = {
        **admin.site.each_context(request),
        'title': f'Отчет по зарплате для {report_user.first_name or report_user.username} за {selected_month:02}.{selected_year}',
        'report_user_display': report_user.first_name if report_user.first_name else report_user.username,
        'selected_year': selected_year,
        'selected_month': selected_month,
        'opening_balance': opening_balance,
        'salary_calculations': salary_calculations,
        'total_accrued_for_current_period': total_accrued_for_current_period,
        'salary_payments': salary_payments,
        'total_paid_for_current_period': total_paid_for_current_period,
...
def get_employee_balance_api(request, employee_id):
    ...
    try: employee_obj = User.objects.get(pk=employee_id)
...
def export_stock_levels_view(request):
    ...
    context = {
        ...
        'site_header': "Администрирование CRM ВелоТема",
    ...
    if 'download_csv' in request.GET:
        ...
        for product in products_data:
            writer.writerow([product.pk, product.name, product.stock_quantity, product.cost_price])
        return response
